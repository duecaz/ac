  ...
  montarHoja(document.getElementById('app'), {
    ronda: {
      ...
      secciones: [{
        titulo: 'Parte 1',
        pruebas: [
          { n: 1, titulo: 'Login', accion: 'Entra con tu usuario.',
...
2. **El texto es el camino base**: funciona sin red, sin permisos y sin backend.
   El envío es un extra que decide el probador, nunca automático.
3. **Si no se puede enviar, se dice ANTES** (`puedeEnviar`), no al fallar.
4. **Un envío fallido se dice** y el texto queda como respaldo (R6 de AulaReto).
5. **El informe incluye lo NO probado**: dice hasta dónde llegó la ronda.
