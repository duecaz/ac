...
| **Ciclo de vida de una pantalla** (relojes, guards, overlays, disposers) | **[`docs/leyes.md`](docs/leyes.md) §23** — regla `reloj-primitivo`; ejemplar: `views/studentLive.js`. El RELOJ y la reanudación son frente aparte, medido y sin ejecutar: **[`docs/handoff-reloj-lifecycle.md`](docs/handoff-reloj-lifecycle.md)** |
...
- **Animaciones IGUALES en todas las pantallas** (no se distingue por aparato: `ww-lite` y su
  ...
  **EXCEPCIÓN DECIDIDA**: la soga del duelo se mece recorriendo ±⅓ de su animación a ~25 repintados/s (`idle()`, core/vsAnimations.js) — cuesta 58 ms/cuadro en la sonda y el dueño lo aceptó con el dato delante (2026-09-14): parado parece colgado. Ni caché de cuadros ni mecer la escena por CSS: probados y rechazados (descuadraban / «se balancean en conjunto»). Si hay que abaratar, se exporta con menos trazados (`docs/handoff-rendimiento-animaciones.md`), no se quita el movimiento. Lo vigilan `tests/animaciones.test.mjs` y `tools/perf-sonda.mjs` (que el DIBUJO cambie · que esté centrado · regresión de orden de magnitud).
...
- Callers (medido 2026-09-04): Wheel, Question-Live, Memory, Match, Wordsearch, Diagram, Ballsort; Tildes y Comas vía `runTextCorrectionSolo` (que corre sobre este shell). **Las 13 sobre un shell** — la migración terminó; este cuadro dijo «pendientes» versiones después de estarlo. **Aquí el marco es de CADA player** (el shell libre no lo posee) y por eso la deuda de §23 vivía aquí: `runTextCorrectionSolo` monta desde v1.51.724 la hoja (`.tc-round` + `edu-cabecera`) UNA vez y cada fase —frase · corrección · frase siguiente— pinta solo `.tc-body`; el interruptor lápiz/borrador es de `textCorrectionRonda.js` (markup **y** cableado) y solo cambia de anfitrión (`cabecera:false` + `mandos`). **Memoria** (v1.51.726) monta el tablero una vez y una jugada cambia UNA CLASE (cada carta nace con sus dos caras): 100 % de nodos vivos, cero remontados — `REHACEN_EL_MARCO` queda VACÍA y así se queda.
