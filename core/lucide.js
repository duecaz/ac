 ...
 *  @param {{clase?: string, tam?: string}} [o]  `clase` extra (opcional) y
 *         tamaño en unidades relativas a la letra (def. `1em`).
 *  @returns {string} el `<svg>`; cadena vacía si el nombre no existe (nunca
 *         rompe la pantalla por un icono, pero se ve que falta). */
export function lucide(nombre, { clase = '', tam = '1em' } = {}) {
  const d = D[nombre];
  if (!d) return '';
  return `<svg class="ww-ico${clase ? ' ' + clase : ''}" width="${tam}" height="${tam}"`
