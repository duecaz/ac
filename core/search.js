...
/** @typedef {{parts: string[], len: number}} Recolector */
...
export function haystackOf(a) {
  ...
  if (T?.meta?.kind !== 'juego') {
    const c = /** @type {Record<string, unknown>} */ (a?.content || {});
...
/**
 * @param {Partial<Activity>|null} activity
 * @param {string} q
 */
 ...
 * @template T
 * @param {T[]|null|undefined} rows   actividades, o filas que las contienen
 * @param {{q?:string, template?:string}} [filter]
 * @param {(x: T) => Partial<Activity>|null|undefined} [pick]  cómo sacar la
 *   actividad de cada elemento (la biblioteca guarda el blob en `row.data`)
 * @returns {T[]} los elementos ORIGINALES que casan
 */
export function searchActivities(rows, filter = {}, pick = (x) => /** @type {Partial<Activity>} */ (x)) {
