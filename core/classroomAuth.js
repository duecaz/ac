...
/** Lo que hace falta de Google Identity Services, que llega por <script> de
 *  Google: frontera, y puede NO estar (sin red).
 *  @typedef {{error?: string, error_description?: string, access_token?: string,
 *    expires_in?: number|string}} RespuestaToken
 *  @typedef {{accounts?: {oauth2?: {initTokenClient: (o: {client_id: string, scope: string,
 *    prompt: string, callback: (resp: RespuestaToken) => void}) => {requestAccessToken: () => void}}}}} GisGlobal */

/** @returns {GisGlobal|null} */
const gis = () => /** @type {GisGlobal|undefined} */ (Reflect.get(globalThis, 'google')) || null;
...
function cached() {
  try {
    const t = JSON.parse(ssGet(CACHE_KEY) || 'null');
...
export async function getClassroomToken({ forceConsent = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!oauth2) { reject(new Error('Google Identity Services no está disponible.')); return; }
